...
describe("package.json", () => {
  ...
  it("check for mandatory fields", () => {
    ...
    if (!configuration.keywords) warn("Missing recommended field: keywords");
    if (!configuration.bugs) warn("Missing recommended field: bugs");
    if (!configuration.license) warn("Missing recommended field: license");
    if (!configuration.author) warn("Missing recommended field: author");
  ...
  it("validating name field", () => {
    ...
    assert.isOk(
      !/^[._]/.test(configuration.name) &&
        /^[\w.~-]+$/.test(configuration.name),
      "valid names may have uppercase and lowercase letters, decimal digits, hyphen, period, underscore, and tilde"
    );

    assert.isOk(
      configuration.name.length < 214,
      "name should be shorter than 214 characters"
    );
    ...
    if (/node/i.test(configuration.name)) {
      warn(
        `You shouldn't use 'node' in the package name, use engines section instead`
      );
    }
    if (/js/i.test(configuration.name)) {
      warn(
        `You shouldn't use 'js' in the package name, use engines section instead`
      );
    }

    console.info(
      `it is always good to check the npm registry to see if the name isn't already taken`
    );
  });

  it("validating version field", () => {
    assert.isOk(
      /^\d+[.]\d+[.]\d+[\w-]*$/.test(configuration.version),
      "expected format [0-9].[0-9].[0-9] (major.minor.patch) optionally followed by [A-Za-z0-9_-]"
    );
  });
  ...
  it("validate main if present", () => {
    if (configuration.main)
      assert.deepEqual(
        typeof configuration.main,
        "string",
        "main should be a string"
      );
  });
  ...
  it("validate keywords if present", () => {
    if (configuration.keywords) {
      assert.deepEqual(
        typeof configuration.keywords,
        "object",
        "keywords should be an array"
      );
      assert.isOk(
        configuration.keywords instanceof Array,
        "keywords should be an array"
      );

      if (configuration.keywords instanceof Array) {
        configuration.keywords.forEach((kw) =>
          assert.deepEqual(
            typeof kw,
            "string",
            `keyword ${kw} should be a string`
          )
        );
      }
    }
  ...
  it("validate bugs if present", () => {
    if (configuration.bugs) {
      if (typeof configuration.bugs !== "string")
        assert.isOk(
          !!(configuration.bugs.url || configuration.bugs.email),
          "bugs should contain url and/or email"
        );
      else
        assert.isOk(
          /^(https?|ftp)/i.test(configuration.bugs),
          "if not an object a valid url is expected"
        );
    }
  });
  ...
  it("validate license if present", () => {
    if (configuration.license) {
      if (typeof configuration.license === "string") {
        assert.isOk(
          /^(ISC|MIT|UNLICENSED|SEE LICENSE IN [\w. -]+)$/.test(
  ...
  it("validate author if present", () => {
    if (configuration.author) {
      if (typeof configuration.author === "string") {
        ...
        assert.deepEqual(
          typeof configuration.author.name,
          "string",
  ...
  it("validate contributors if present", () => {
    if (configuration.contributors) {
      assert.isOk(
        configuration.contributors instanceof Array,
  ...
  it("validate preferGlobal if present", () => {
    if (typeof configuration.preferGlobal !== "undefined") {
      assert.deepEqual(
        typeof configuration.preferGlobal,
        "boolean",
        "preferGlobal should be a boolean"
      );
    }
  });
    ...
    if (configuration.os) {
      ...
      if (configuration.os instanceof Array) {
        configuration.os.forEach((o) => {
          ...
          if (typeof o === "string") assert.isOk(!!o.replace(/\s/g, "").length);
        });
      }
    }
  });
  ...
  const validateDependency = (dp) => {
    ...
    for (let key of keys) {
      ...
      if (/^(>=?|<=?|[~^])?\d+[.]\d+[.]\d+[\w-]*$/.test(dependency))
        ...
        /^\d+[.]\d+[.]\d+[\w-]*\s*-\s*\d+[.]\d+[.]\d+[\w-]*$/.test(dependency)
      )
        isValid = true;
